...
class UserAccsesForm extends React.Component{
    ...
    render(){
      return (
          ...
          <ActionBox text="Открыть для всех" icon={<Link45deg />} action={()=>{this.setState({...this.state, checkeveryone:!this.state.checkeveryone}); axios.put('http://127.0.0.1:5000/share_conspect_to_all/'+this.props.conspectid)}}/>
          <Dropdown>
          ...
          </>:<ActionBox text="Закрыть для всех" icon={<Link45deg />} action={()=>{this.setState({...this.state, checkeveryone:!this.state.checkeveryone}); axios.put('http://127.0.0.1:5000/set_conspect_private/'+this.props.conspectid)}}/>
      )
    }
}
